...
public class ContentSecurityConfiguration {
    ...
    public ContentSecurityConfiguration(ContentSecurityProperties properties) {
    ...
    void validate() {
        if (properties.getSecret() == null || properties.getSecret().isBlank()) {
            throw new TechnicalException(
                    ...
                    "Content JWT secret is required"
            );
        }
        if (properties.getSecret().getBytes(StandardCharsets.UTF_8).length < 32) {
            throw new TechnicalException(
                    AppErrorCode.AUTH_PROVIDER_CONFIGURATION_INVALID,
                    "Content JWT secret must be at least 32 bytes"
            );
        }
    }
    ...
    SecurityFilterChain contentSecurityFilterChain(
            HttpSecurity http,
            @Qualifier("contentCorsConfigurationSource") CorsConfigurationSource corsConfigurationSource
    ) throws Exception {
        return http
                ...
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/health", "/actuator/health", "/actuator/info", "/actuator/prometheus")
                        .permitAll()
                        .requestMatchers(HttpMethod.GET, "/api/v1/posts/published").authenticated()
    ...
    @Bean
    CorsConfigurationSource contentCorsConfigurationSource(HttpProperties properties) {
