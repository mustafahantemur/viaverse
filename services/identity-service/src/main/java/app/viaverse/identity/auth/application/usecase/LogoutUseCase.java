...
public class LogoutUseCase {
    ...
    @Transactional
    public void logout(JwtPrincipal principal, String refreshToken) {
        ...
        IdentityAuditEvents.recordAccountSecurityEvent(auditLogger, session.getAccountId(), IdentityAuditEvent.SESSION_REVOKED);
        ActionLogContext.put("auth.session_id", session.getId());
        ActionLogContext.put("user.id", session.getAccountId());
    }
}
