...
public class AuthAbuseProtectionService {
    ...
    public void enforceStart(NormalizedIdentifier normalized, String clientIp, String clientFingerprint, Instant now) {
        ...
        check(RateLimitScope.AUTH_START_IDENTIFIER, normalized.type() + ":" + normalized.value(),
                authStart.getIdentifierMaxAttempts(), Duration.ofSeconds(authStart.getIdentifierWindowSeconds()));
        check(RateLimitScope.AUTH_START_IP, clientIp,
                authStart.getIpMaxAttempts(), Duration.ofSeconds(authStart.getIpWindowSeconds()));
        check(RateLimitScope.AUTH_START_DEVICE, clientFingerprint,
                authStart.getIpMaxAttempts(), Duration.ofSeconds(authStart.getIpWindowSeconds()));
    ...
    public void enforceOtpAttempt(AuthLoginFlow flow, String clientIp) {
        ...
        check(RateLimitScope.OTP_VERIFY_FLOW, flowKey,
                otpVerify.getFlowMaxAttempts(), Duration.ofSeconds(otpVerify.getFlowWindowSeconds()));
        check(RateLimitScope.OTP_VERIFY_IDENTIFIER, identifierKey,
                otpVerify.getFlowMaxAttempts(), Duration.ofSeconds(otpVerify.getFlowWindowSeconds()));
        check(RateLimitScope.OTP_VERIFY_IP, clientIp,
    ...
    public void softLockOtp(AuthLoginFlow flow) {
        ...
        check(RateLimitScope.OTP_VERIFY_IDENTIFIER,
    ...
    private void check(RateLimitScope scope, String key, int limit, Duration window) {
