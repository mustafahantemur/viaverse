...
public class MediaSecurityConfiguration {
    ...
    private final MediaSecurityProperties properties;

    public MediaSecurityConfiguration(MediaSecurityProperties properties) {
    ...
    void validate() {
        if (properties.getSecret() == null || properties.getSecret().isBlank()) {
            throw new TechnicalException(
                    ...
                    "Media JWT secret is required"
            );
        }
        if (properties.getSecret().getBytes(StandardCharsets.UTF_8).length < 32) {
            throw new TechnicalException(
                    AppErrorCode.AUTH_PROVIDER_CONFIGURATION_INVALID,
                    "Media JWT secret must be at least 32 bytes"
            );
        }
    }
    ...
    SecurityFilterChain mediaSecurityFilterChain(
            HttpSecurity http,
            @Qualifier("mediaCorsConfigurationSource") CorsConfigurationSource corsConfigurationSource
    ) throws Exception {
        return http
    ...
    @Bean
    CorsConfigurationSource mediaCorsConfigurationSource(HttpProperties properties) {
