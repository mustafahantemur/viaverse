...
- Secrets and connection details are never committed. This includes passwords, tokens, API keys, OAuth client secrets, SMTP/SMS credentials, database URLs, usernames/passwords, object-storage keys, broker/cache endpoints, and production/internal service URLs.
- Docker Compose may use ignored `.env.local` files for local-only development. Kubernetes workloads must source secret and connection values from Vault; ConfigMaps are only for non-sensitive toggles such as profile/environment names.
- Do not commit Kubernetes `Secret` manifests with real values. Bootstrap Vault from an ignored env file or CI/CD secret store, then let Vault Agent render the runtime env file for pods.
